...
export const updateSensorHandler = Route.asyncHandler(async (req, res) => {
    const s_id = +req.params.s_id;
    const { s_index, s_addr, s_threshold_warning, s_threshold_critical, s_threshold_active, st_id } = req.body;
    ...
    const sensor = await Sensor.updateByPk(s_id, { s_index, s_addr, s_threshold_warning, s_threshold_critical, s_threshold_active, st_id });
